...
import wandb
wandb.login(key="e38fa12175f853d2e175")
...
def run_validation(model, val_loader, accelerator, device, criterion, total_frames_in_window):
    model.eval()
    ...
    with torch.no_grad():
        ...
        for data in val_bar:
            ...
            real_batch_size = flat_batch_size // total_frames_in_window

            frames = frames.view(real_batch_size, total_frames_in_window, *frames.shape[1:])
            labels = labels.view(real_batch_size, total_frames_in_window, -1)[:, 0, 0]
...
optimizer = torch.optim.AdamW([
    ...
    {"params": model.lstm.parameters(), "lr": temporal_lr},
