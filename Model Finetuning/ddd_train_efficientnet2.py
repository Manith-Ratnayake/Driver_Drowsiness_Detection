...
train_loader = DataLoader(train_dataset, batch_size=12, shuffle=True, num_workers=1, pin_memory=True)
val_loader   = DataLoader(validation_dataset, batch_size=12, shuffle=False, num_workers=1, pin_memory=True)
test_loader  = DataLoader(test_dataset, batch_size=12, shuffle=False, num_workers=1, pin_memory=True)
...
path = f"/kaggle/working/EfficientNet_v2_Large_epoch_{epochs_resume_no}.pth"
...
for epoch in range(start_epoch, epochs):
    ...
    if accelerator.is_main_process:
        ...
        with open("training_details_EfficientNetV2Large_exp2.txt", "a") as f:
            f.write(log_line + "\n")
        ...
        accelerator.save(
            ...
            f"EfficientNet_v2_Large_epoch_{epoch}.pth",
        )
...
if accelerator.is_main_process:
    log_test_line = f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} -  Final Test: Loss {test_loss:.4f}, Acc {test_acc:.2f}%"
    print(log_test_line)
